...
subscale_info_1<- global_subscale_info[["SDS"]] #Subset the global list to retrive the subscale list(s) for this particular measure
                ...
                fluidRow(
                  ...
                  tabBox(id = "tabset", width = 12,
                         ...
                         tabPanel(psychlytx::simplified_show_name_UI("show_name"),
                                  ...
                                  psychlytx::simplified_sds_scale_UI("sds_scale"), #Item of the specific measure
                                  
                                  psychlytx::manual_data_UI("manual_data"), #Items of the specific measure are passed here as a string of numbers
                                  
                                  psychlytx::format_sds_responses_for_email_UI("format_responses_for_email"),
...
server <- function(input, output, session) {
  ...
  scale_entry<- callModule(psychlytx::simplified_sds_scale, "sds_scale", client_id) #Return the raw responses to the online scale
                                                                                 #Pass in client_id to allow resetting
  
  manual_entry<- callModule(psychlytx::manual_data, "manual_data", scale_entry, expected_responses = 5) #Raw item responses are stored as vector manual_entry to be used downstream
