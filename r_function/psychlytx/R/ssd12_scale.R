...
#' @export
#'
ssd12_scale<- function(input, output, session, selected_client = NULL, simplified = FALSE) {
  ...
  observe({
    ...
    item_too_long<- purrr::map_lgl(list(c(input$item_1), c(input$item_2), c(input$item_3), c(input$item_4),
                                        c(input$item_5), c(input$item_6), c(input$item_7),
                                        c(input$item_8), c(input$item_9), c(input$item_10), c(input$item_11),
                                        c(input$item_12)), ~length(.x) > 1)
