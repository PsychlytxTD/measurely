#' Convert SIAS Measure Responses
#'
#' Convert SIAS raw measure responses into a readable table, to be send in an email to the clinician.
#'
...
format_sias_responses_for_email_UI<- function(id) {
...
format_sias_responses_for_email<- function(input, output, session, pool, clinician_email, manual_entry, measure_data, simplified = FALSE) {
  ...
  reactive({
    ...
    body<- do.call(sprintf, c(list('{"from": {"email":"measurely@psychlytx.com","name":"Measurely"},
                                   ...
                                   "template_id":"d-2591629390784b62ba89cb3e41c63305"}'), body_values)) #Pass in the vector of strings to replace placeholders in order.
