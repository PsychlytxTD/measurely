#' Convert PHQ-9 Measure Responses
#'
#' Convert PHQ-9 raw measure responses into a readable table, to be send in an email to the clinician.
#'
...
format_phq9_responses_for_email_UI<- function(id) {
...
format_phq9_responses_for_email<- function(input, output, session, pool, clinician_email, manual_entry, measure_data, simplified = FALSE) {
  ...
  reactive({
    ...
    severity_range<- dplyr::case_when(
      ...
      measure_data()$score < 10 ~ "Sub-threshold",
