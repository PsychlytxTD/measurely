#' Convert PCL-5 Measure Responses
#'
...
format_pcl5_responses_for_email_UI<- function(id) {
...
format_pcl5_responses_for_email<- function(input, output, session, pool, clinician_email, manual_entry, measure_data, simplified = FALSE) {
  ...
  reactive({
    ...
    simplified_cutoff_descriptor_total<- dplyr::case_when(
      ...
      as.numeric(score_severity_range[1]) < 33 ~ "Below PTSD threshold",

      as.numeric(score_severity_range[1]) >= 33 ~ "At/Above PTSD threshold",
    ...
    score_severity_range[6]<- simplified_cutoff_descriptor_total    #Replace the original cutoff descriptions with the simplified info for the two scales
