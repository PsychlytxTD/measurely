#' Convert SSD-12 Measure Responses
#'
#' Convert SSD-12 raw measure responses into a readable table, to be send in an email to the clinician.
#'
...
format_ssd12_responses_for_email_UI<- function(id) {
...
format_ssd12_responses_for_email<- function(input, output, session, pool, clinician_email, manual_entry, measure_data, simplified = FALSE) {
  ...
  reactive({
    ...
    formatted_item_responses<- dplyr::case_when( #Convert the client's responses from numerical form to readable responses, teo appear in the email.
      ...
      manual_entry()$item_scores == 0 ~ "Never",

      manual_entry()$item_scores == 1 ~ "Rarely",

      manual_entry()$item_scores == 2 ~ "Sometimes",

      manual_entry()$item_scores == 3 ~ "Often",

      manual_entry()$item_scores == 4 ~ "Very Often",
    ...
    body<- do.call(sprintf, c(list('{"from": {"email":"measurely@psychlytx.com","name":"Measurely"},
                                   ...
                                   "template_id":"d-22f17a11a0a74a42829adec4e27bf739"}'), body_values)) #Pass in the vector of strings to replace placeholders in order.
